...
- **RESOLVED (Step 6): the handoff works, no fallback needed.** A real grab flowed end-to-end (Livrarr grab → SAB → import → CWA ingest → `/mnt/nas/Ebooks/<Author>/<Title>/` → library → Send to Kindle). **No re-grab loop** — Livrarr tracks the book's status in its own DB from the import event, not by polling disk, so CWA deleting the file did not flag it "missing." Livrarr writes into its `1/` user-namespace dir (`/downloads/ebook-ingest/1/<Author>/<Book>/`), and **CWA's recursive ingest watcher (`-r`) picks files up from inside it** — so the `1/` nesting is a non-issue. One cosmetic residue: an **empty `1/` directory** is left in the ingest folder after import (CWA removes the file + `<Author>/<Book>` subdirs but not the top `1/`). It's a single stable dir, does not accumulate, harmless.
