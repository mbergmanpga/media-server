...
- **NAS mount:** `/mnt/nas/{Television,Movies,Music,Audiobooks,Ebooks,Backups}` — all *arr containers, Plex, ABS, and CWA bind-mount these. The share is **CIFS/SMB** (`//10.0.0.58/share`), and it must be mounted with **`nobrl`** in `/etc/fstab`. CIFS can't honor SQLite byte-range locks, so any SQLite DB on the NAS (notably CWA's `metadata.db`) throws `apsw.BusyError: database is locked` on writes without it. If a container can't see media, check the host mount first (`df -h | grep nas`); if a Calibre/SQLite write fails on the NAS, check `findmnt /mnt/nas | grep nobrl`.
